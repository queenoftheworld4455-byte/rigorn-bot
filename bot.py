...
BROCHURE_TEXTS = {
    ...
    "guide1": """
Гайд для инвестора: каждый застройщик хочет привлечь к своим объектам внимание инвесторов, а вместе с тем и их средства.

Обещания роста в цене на 50% по окончании строительства, красочные рендеры, удобный план рассрочки: всё это требует проверки, потому что на кону Ваши средства, а самое главное — время.

Гайд рассматривает объекты в комплексе: от управляющей компании до наличия социальной и транспортной инфраструктуры.

Мы создавали этот материал для того, чтобы Вы смогли сами по косвенным признакам определить перспективы проектов.
""",

    "Brochure": """
10 самых частых ошибок инвесторов: недвижимость это скучно. Нужно смотреть на факты, реальные кейсы и оценивать возможности конкретного объекта, в то же время сравнивая его со многими другими.

Но именно такой подход к инвестициям поможет сохранить, а затем и приумножить капитал.

Смотрим на реальные решения и их последствия без розовых очков, а затем сравниваем с проектами, которые действительно стоят внимания.
"""
}
...
каталог будет отправлен автоматически.
"""
    )
